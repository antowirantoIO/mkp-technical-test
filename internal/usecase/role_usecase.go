...
func (c *RoleUseCase) Delete(ctx context.Context, request *model.DeleteRoleRequest) error {
	...
	if err := c.RolePermissionRepository.DeleteAllByRoleID(tx, role.ID); err != nil {
...
func (c *RoleUseCase) List(ctx context.Context, request *model.ListRoleRequest) (*model.WebResponse[[]model.RoleResponse], error) {
...
func (c *RoleUseCase) AssignPermissions(ctx context.Context, request *model.AssignPermissionsRequest) error {
	tx := c.DB.WithContext(ctx).Begin()
	defer tx.Rollback()
	...
	for _, permissionID := range request.PermissionIDs {
		...
		existing := &entity.RolePermission{}
		if err := c.RolePermissionRepository.FindByRoleIDAndPermissionID(tx, existing, request.RoleID, permissionID); err == nil {
		...
		if err := c.RolePermissionRepository.Create(tx, rolePermission); err != nil {
			c.Log.WithError(err).Error("failed to create role permission")
			return fiber.ErrInternalServerError
		}
	}
...
func (c *RoleUseCase) RemovePermissions(ctx context.Context, request *model.RemovePermissionsRequest) error {
	tx := c.DB.WithContext(ctx).Begin()
	defer tx.Rollback()
	...
	for _, permissionID := range request.PermissionIDs {
		if err := c.RolePermissionRepository.DeleteByRoleIDAndPermissionID(tx, request.RoleID, permissionID); err != nil {
			c.Log.WithError(err).Error("failed to delete role permission")
			return fiber.ErrInternalServerError
		}
	}
